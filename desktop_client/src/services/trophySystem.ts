...
export const TROPHIES: Trophy[] = [
  ...
  // GOLD (6)
  {
    ...
    tier: 'gold',
    title: '阅片浩瀚',
    desc: '累计探索浏览 1,000 部不同影片',
    icon: 'Film',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.uniqueMoviesViewed.length, 1000), max: 1000 }),
    condition: (a) => a.uniqueMoviesViewed.length >= 1000,
  },
  {
    id: 'gold_streak_30',
    tier: 'gold',
    title: '岁月沉浸',
    desc: '在客户端中累计活跃探索打卡 30 天',
    icon: 'Calendar',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.activeDays.length, 30), max: 30 }),
    condition: (a) => a.activeDays.length >= 30,
  },
  {
    id: 'gold_performer_500',
    tier: 'gold',
    title: '星河浩荡',
    desc: '累计了解探索 500 位不同演员档案',
    icon: 'Users',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.uniquePerformersViewed.length, 500), max: 500 }),
    condition: (a) => a.uniquePerformersViewed.length >= 500,
  },
  {
    id: 'gold_favorite_200',
    tier: 'gold',
    title: '挚爱珍藏',
    desc: '个人收藏项目累计达到 200 项',
    icon: 'Heart',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.favoritesAddedCount, 200), max: 200 }),
    condition: (a) => a.favoritesAddedCount >= 200,
  },
  {
    id: 'gold_focus_100h',
    tier: 'gold',
    title: '时光胶囊',
    desc: '在影视库中累计活跃专注时长达 100 小时',
    icon: 'Clock',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(Math.floor(a.totalFocusTimeSeconds / 3600), 100), max: 100 }),
    condition: (a) => a.totalFocusTimeSeconds >= 360000,
  },
  {
    id: 'gold_rating_100',
    tier: 'gold',
    title: '权威鉴赏家',
    desc: '为 100 部影片完成私密评星打分',
    icon: 'Star',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.ratingsCount, 100), max: 100 }),
    condition: (a) => a.ratingsCount >= 100,
  },
  ...
  // SILVER (20)
  {
    ...
    icon: 'Film',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.uniqueMoviesViewed.length, 100), max: 100 }),
    condition: (a) => a.uniqueMoviesViewed.length >= 100,
  },
  {
    id: 'silver_movie_300',
    tier: 'silver',
  ...
  },
  {
    ...
    icon: 'Flame',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.uniqueMoviesViewed.length, 500), max: 500 }),
    condition: (a) => a.uniqueMoviesViewed.length >= 500,
  },
  {
    id: 'silver_episode_100',
    tier: 'silver',
    title: '片段捕手',
    desc: '累计查看鉴赏 100 个场景片段',
    icon: 'Layers',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.episodeViewsCount, 100), max: 100 }),
    condition: (a) => a.episodeViewsCount >= 100,
  },
  {
    id: 'silver_episode_300',
    tier: 'silver',
    title: '分集大观',
    desc: '累计查看鉴赏 300 个场景片段',
    icon: 'Tv',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.episodeViewsCount, 300), max: 300 }),
    condition: (a) => a.episodeViewsCount >= 300,
  },
  {
    ...
    icon: 'Users',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.uniquePerformersViewed.length, 50), max: 50 }),
    condition: (a) => a.uniquePerformersViewed.length >= 50,
  },
  {
    ...
    icon: 'Sparkles',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.uniquePerformersViewed.length, 100), max: 100 }),
    condition: (a) => a.uniquePerformersViewed.length >= 100,
  },
  {
    ...
    icon: 'Award',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.uniquePerformersViewed.length, 250), max: 250 }),
    condition: (a) => a.uniquePerformersViewed.length >= 250,
  },
  {
    ...
    icon: 'Megaphone',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.directorViewsCount, 20), max: 20 }),
    condition: (a) => a.directorViewsCount >= 20,
  },
  {
  ...
  },
  {
    ...
    icon: 'Building2',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.studioViewsCount, 20), max: 20 }),
    condition: (a) => a.studioViewsCount >= 20,
  },
  {
    id: 'silver_studio_50',
    tier: 'silver',
    ...
    icon: 'Landmark',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.studioViewsCount, 50), max: 50 }),
    condition: (a) => a.studioViewsCount >= 50,
  },
  {
    id: 'silver_search_50',
    tier: 'silver',
    title: '灵动探索引擎',
    desc: '全库执行精准检索达到 50 次',
    icon: 'Search',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.searchesCount, 50), max: 50 }),
    condition: (a) => a.searchesCount >= 50,
  },
  {
  ...
  },
  {
    ...
    icon: 'Bookmark',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.favoritesAddedCount, 50), max: 50 }),
    condition: (a) => a.favoritesAddedCount >= 50,
  },
  {
    ...
    icon: 'Gem',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.favoritesAddedCount, 100), max: 100 }),
    condition: (a) => a.favoritesAddedCount >= 100,
  },
  {
    id: 'silver_rating_30',
    tier: 'silver',
    ...
    icon: 'Star',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.ratingsCount, 30), max: 30 }),
    condition: (a) => a.ratingsCount >= 30,
  },
  {
    id: 'silver_rating_50',
    tier: 'silver',
    ...
    icon: 'CheckCircle',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.ratingsCount, 50), max: 50 }),
    condition: (a) => a.ratingsCount >= 50,
  },
  {
    id: 'silver_focus_10h',
    tier: 'silver',
    title: '专注凝视',
    desc: '累计活跃探索时长达到 10 小时',
    icon: 'Hourglass',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(Math.floor(a.totalFocusTimeSeconds / 3600), 10), max: 10 }),
    condition: (a) => a.totalFocusTimeSeconds >= 36000,
  },
  {
    id: 'silver_streak_7',
    tier: 'silver',
    title: '一周之约',
    desc: '在客户端中连续打卡活跃 7 天',
    icon: 'CheckCheck',
    unlockedAt: null,
    progress: (a) => ({ current: Math.min(a.activeDays.length, 7), max: 7 }),
    condition: (a) => a.activeDays.length >= 7,
  },
...
export function triggerTrophyUnlock(trophy: Trophy) {
  ...
  if (pluginsConfig.value.trophiesEnabled) {
    // Play Web Audio Pure Algorithmic Chime
