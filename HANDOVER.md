...
- **★ 导演库的界面还没人点过（本轮最大的缺口）。** 接口、查询、类型检查、跨实现对拍
  都验过了，但**桌面端的界面只做到「编译和类型检查通过」**，没有实际跑起来点一遍。
  待点：导演库首屏/搜索/排序/翻页/滑到底加载、点开导演页、作品关键字与年份筛选、
  片商 chip 跳过去之后**片商库的搜索框里有没有填上那家片商**（`openStudioLibrary()`
  里先设 `currentTab` 再设 `studioQuery` 是有顺序要求的，顺序反了 watch 不触发、
  搜索框会是空的——这一条最值得先看）、收藏一位导演后到「我的收藏」看计数。
  代码路径都写好了，但「写好了」和「点通了」在这次之间没有证据。
