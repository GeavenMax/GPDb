...
1. 下载跑完 ✅ → 2. **同一命令再跑三遍**（只重试缺的）✅ → 3. `--revert-missing-hd --apply` ✅
   → 4. `--prune-orphans --apply` ✅

⚠️ **`--revert-missing-hd` 和 `--prune-orphans` 是 flag，不是 `--mode` 的取值。**
写成 `--mode revert-missing-hd` 会被 argparse 直接拒掉。各自先不带 `--apply` 跑一次看数量。
...
PNG 368、WebP 151、GIF 3——站点在 `.jpg` 这个 URL 下会回别的格式，文件本身结构完整
（PNG 有 `IHDR`+`IEND`、WebP 的 RIFF 长度对得上）。浏览器按字节嗅探，显示应无问题；
但如果有谁依赖「扩展名 = 真实格式」，这里会翻车。
...
本轮把这件事量到底了：第一遍 **11,730 个失败里 11,726 个是「图在、我们没拿到」**
（四遍跑完只剩 4 条真的没有，**99.97%**）。**照最直觉的三步走直接回滚，
这 11,726 集会被永久降级成低清。** 重跑（幂等、只碰缺的）之后仍然缺的，才基本是站点真的没有。
...
14. **跳过规则会把一次性坏响应永久固化**：`download_image` 的跳过条件是
    「文件存在且 >100 字节」，**不看内容**。所以任何一次落到盘上的坏响应
