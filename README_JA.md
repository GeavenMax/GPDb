...
> 📢 **公式 Telegram チャンネル**: 最新のリリース情報、データベース更新通知、使い方のヒントについては [公式 GPDb Telegram チャンネル (https://t.me/gpdbnews)](https://t.me/gpdbnews) を購読してください！
...
┌──────────────────────────────▼──────────────────────────────┐
│                   永続化ストレージレイヤー                  │
│      GPDb.db (SQLite 3 WAL モード) + image_cache/ (ローカル) │
└─────────────────────────────────────────────────────────────┘
```
...
- **macOS**：`GPDb-macOS-v2.6.0.dmg` をダウンロードし、マウント後に `GPDb.app` を `Applications` フォルダへドラッグ＆ドロップします。
